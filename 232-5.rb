...
class World
  ...
  def initialize(nodes=nil, current_node_name=nil, path_length=0, visited_nodes=[])

    @finished = false
    @success = nil
    @path_length = path_length
    @nodes = {}
    @visited_nodes = visited_nodes.dup
  ...
  def step
    ...
    if end_node(@current_node) # we did it!
      ...
      nabes = @current_node.edges.to_a.filter {|p| ! @visited_nodes.include?(p[0]) }
      ...
      if nabes.length == 0 # dead end
        ...
        me = nabes.shift
        ret = nabes.map { |n| World.new(@nodes, n[0], @path_length + n[1], @visited_nodes) }
...
while multiverse.length > 0 do
  spax += 1
  puts("MVL: #{multiverse.length} Successes: #{success_runs.length} #{success_runs.max} ") if spax % 1000 == 0
  w = multiverse.shift
  until w.finished do
